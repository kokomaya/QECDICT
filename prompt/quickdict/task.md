...
Task 1 (脚手架)
  ...
  Task 20 (dxcam截图回退) ── 依赖 Task 9  ✅
  Task 21 (多显示器修复)  ── 依赖 Task 9  ✅
  Task 22 (Y加权选词)     ── 依赖 Task 9  ✅
  Task 23 (截图区域可视化) ── 依赖 Task 9 ✅
  Task 24 (状态指示器)    ── 依赖 Task 8  ✅
...
| 19a  | OCR增强: OpenCV依赖+截图扩大 | ✅ 已完成  | 依赖 Task 9 |
| 19b  | OCR增强: 图像预处理管线      | ✅ 已完成  | 依赖 19a |
| 19c  | OCR增强: 多策略重试+参数调优 | ✅ 已完成  | 依赖 19b |
| 19d  | OCR增强: 倾斜文字校正        | ⬜ 未开始  | 可选，依赖 19b |
| 20   | dxcam 截图回退（硬件加速窗口）| ✅ 已完成  | Teams 等 DXGI 窗口 |
| 21   | 多显示器坐标修复       | ✅ 已完成  | 负坐标/副屏支持 |
| 22   | OCR 文本框 Y 加权选词  | ✅ 已完成  | 优先同行文字 |
| 23   | 截图区域可视化（调试用）| ✅ 已完成  | 托盘开关控制 |
| 24   | 取词状态指示器         | ✅ 已完成  | 托盘开关，取词中常显 |
| 16   | 划词翻译               | ⬜ 未开始  |      |
| 17   | 全局搜索框             | ❌ Reject  |      |
